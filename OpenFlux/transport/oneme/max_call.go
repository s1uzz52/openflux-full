...
func (h *CallHandler) createPeerConnection(convParams map[string]interface{}) {
	// With ICE injection the MAX signaling WebSocket is the packet transport.
	// Avoid opening unused WebRTC sockets, which would need separate VPN bypass
	// handling and are not part of the packet path in this mode.
	if useICEInjection {
		logInfo("[%s] Signaling packet channel ready", h.tag)
...
func startOutgoingCall(client *MaxClient, calleeID int64, autoReconnect bool) *CallHandler {
	...
	go func() {
		for h.running.Load() {
			...
			dialer := websocket.Dialer{NetDialContext: transport.ProtectedDialContext}
