...
func authorize(docURL string) (*volgaAuth, error) {
	...
	session := &http.Client{
		...
		Transport: &http.Transport{
			DialContext:         transport.ProtectedDialContext,
			MaxIdleConns:        100,
...
func (w *wsListener) connect() error {
	...
	// NetDialContext routes through the Android VpnService socket-protect
	// path (no-op on desktop builds) so the WS control connection doesn't
	// get recursively captured by the tunnel's own TUN interface.
